...
async function main() {
  ...
  async function createScene() {
    ...
    // カメラがユーザからの入力で動くように
    camera.attachControl(canvas, true, undefined, true);
    camera.setPosition(new BABYLON.Vector3(0, 0, -10));
    //並行投影の設定 //https://doc.babylonjs.com/typedoc/classes/BABYLON.ArcRotateCamera#ORTHOGRAPHIC_CAMERA
    camera.mode = BABYLON.Camera.ORTHOGRAPHIC_CAMERA;
    const cameraDistance = 10;
    const cameraRatio = canvas.height / canvas.width;
    camera.orthoLeft = -cameraDistance / 2;
    camera.orthoRight = cameraDistance / 2;
    camera.orthoTop = camera.orthoRight * cameraRatio;
    camera.orthoBottom = camera.orthoLeft * cameraRatio;
    ...
    async function loadGUI() {
      ...
      guiTex = GUI.AdvancedDynamicTexture.CreateFullscreenUI(
        "GUI",
        true,
        scene
      );
      // button1 = GUI.Button.CreateSimpleButton("but1", "Click Me");
      // button1.width = "550px";
      // button1.height = "400px";
      // button1.color = "white";
    ...
    //svgからオブジェクトを作る
    mountainMesh = await createExtrudedShapeFromSvg(
      "mountain",
      "./assets/mountain.svg",
      {
        closeShape: true,
      ...
      0,
      scene,
      (extrudedShape, shapeArray) => {
        console.log("Extruded shape created:", extrudedShape);
        console.log({ shapeArray });
        mountainShape = shapeArray;
      }
    );

    console.log("mountain position ");
    console.log(mountainMesh.getBoundingInfo().boundingBox.maximum.x); //mountain.getBoundingInfo().boundingBoxの成分を参照できる。
    let mountainX =
      -1 * (mountainMesh.getBoundingInfo().boundingBox.maximum.x / 2);
    let mountainY =
      -1 * (mountainMesh.getBoundingInfo().boundingBox.minimum.y / 2);
    let mountainPos = new BABYLON.Vector3(mountainX, mountainY, 0);
    console.log(mountainPos);
    mountainMesh.setPositionWithLocalVector(mountainPos); //setPositionWithLocalVector(vector3配列)で新しいポジションを指定できる。

    mountainMesh.physicsImpostor = new BABYLON.PhysicsImpostor(
      mountainMesh,
      BABYLON.PhysicsImpostor.MeshImpostor,
      { mass: 0, friction: 0.5, restitution: 0.7 },
      scene
    ...
    river = await createExtrudedShapeFromSvg(
      "river",
    ...
    riverTransform = new BABYLON.TransformNode("riverTransform", scene);
...
async function createExtrudedShapeFromSvg(
  ...
  if (isLine === 1) {
    ...
    myPath = [new BABYLON.Vector3(0, 0, 0), new BABYLON.Vector3(0, 0, 0.01)];
    // ExtrudeShapeオプションのshape,pathプロパティを設定
    extrusionOptions.shape = myShape;
    extrusionOptions.path = myPath;
    console.log(myShape);
  ...
  // コールバック関数を実行
  if (callback) {
    callback(extrudedShape, shapeArray);
  }
